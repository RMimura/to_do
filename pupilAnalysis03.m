    ...
    for bb = 1:4
        ...
        for block = 1:length([Data.condition])
            for trl = 1:length(Data(block).TR)
                nn = nn + 1;
                dDurIdx(nn)...
                    = round((Data(1).TR(1).pSOA+2)*1000/120);
            end
        end
        clear nn block trl
        
        nn=0;empIdx = [];
        for trl=1:MSGtime(end,1) % The last row, 1st col; the num of trial (RM)
            
            if pp == 'y' % plot trial data
                % Figure setting
                if mod(trl,20)==1 %amari of trl/20 (RM) 
                    nn=nn+1;
                    h = figure(...
                        'InvertHardcopy', 'off',...
                        'Color', [1 1 1],...
            ...
            c.dmtime = MSGtime(MSGtime(:,1)==trl & MSGtime(:,3)==20,2); % Disk mask presentation
            
            if isempty(c.dtime)
                c.dtime = MSGtime(MSGtime(:,1)==trl & MSGtime(:,3)==1,2)+8;
            elseif isempty(c.dmtime)
                c.dmtime = MSGtime(MSGtime(:,1)==trl & MSGtime(:,3)==1,2)+dDurIdx(trl); % dDurIdx(trl) means additional time from "letter"
            end
            ...
            if c.nData < 50
                ...
                if isempty(c.ppldat(c.ppldat(:,1)==0,2))
                    c.dilation = NaN;
                    c.ppldat(:,2) = NaN;
                else
                    c.dilation...
                        = (c.ppldat(c.ppldat(:,1)==c.dmtime - c.dtime,2)...
                        -c.ppldat(c.ppldat(:,1)==0,2))/c.ppldat(c.ppldat(:,1)==0,2);
                    c.ppldat(:,2) = (c.ppldat(:,2)-c.ppldat(c.ppldat(:,1)==0,2))/c.ppldat(c.ppldat(:,1)==0,2);
                end
        ...
        % Create summary data with -100ms to 300ms range
...
%Mean of all data
D.mData(:,1) = sdat:edat;
D.mData(:,2) = nanmean(D.mean.single_c(:,find(idx==sdat):find(idx==edat)));
D.mData(:,3) = nanmean(D.mean.single_p.trained(:,find(idx==sdat):find(idx==edat)));
D.mData(:,4) = nanmean(D.mean.single_p.untrained(:,find(idx==sdat):find(idx==edat)));
